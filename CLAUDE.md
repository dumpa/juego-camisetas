...
- **`src/analisis.js`** — las cuentas del vistazo a los datos (`DatosView`): funciones puras sobre el estado, fuera de `App.jsx` para poder correrlas desde Node (`tests/analisis.test.mjs`). Tres reglas que parecen detalles y no lo son: las completadas se cuentan desde `movimientos` (los eventos no se borran al deshacer), las camisetas donadas siguen contando (su nombre, `created_at` y tonos viven en el evento `camiseta_donada`), y los días se cortan en **hora local**, no en UTC. **La racha y el porcentaje de días activos solo pueden existir dentro de esta vista** — es una excepción acotada a la regla dura de `docs/brief.md`, con su razón en `docs/decisiones.md`; no se propaga a ecos, rituales ni ninguna pantalla del juego.
